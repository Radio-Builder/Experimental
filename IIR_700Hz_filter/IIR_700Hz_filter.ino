...
// Filter coefficients for 8 biquads (16th-order filter)
float a0_1, a1_1, a2_1, b0_1, b1_1, b2_1;  // Coefficients for all biquads (same)
float x1[8] = {0}, x2[8] = {0}, y1_prev[8] = {0}, y2_prev[8] = {0};  // States for 8 biquads
...
void loop() {
    ...
    // Apply the eight biquads to only the left channel (skip right channel samples)
    for (int i = 0; i < num_samples; i += 2) {  // Step by 2 to only process left channel
        ...
        // Apply all eight biquads in cascade
